...
public class OrderApprovalSaga implements SagaStep<RestaurantApprovalResponse> {
    ...
    public OrderApprovalSaga(OrderDomainService orderDomainService,
                             OrderSagaRepositoryHelper orderSagaHelper,
    ...
    public void process(RestaurantApprovalResponse restaurantApprovalResponse) {
        final var orderApprovalOutboxResponse =
        ...
        final var orderApprovalOutboxMessage = orderApprovalOutboxResponse.get();
        final var order = approveOrder(restaurantApprovalResponse);
        final var sagaStatus =
    ...
    public void rollback(RestaurantApprovalResponse restaurantApprovalResponse) {
        final var approvalOutboxMessageResponse = approvalOutboxHelper.getApprovalOutboxMessageBySagaIdAndSagaStatus(
                UUID.fromString(restaurantApprovalResponse.getSagaId()),
                SagaStatus.PROCESSING
        );
        if (approvalOutboxMessageResponse.isEmpty()) {
            log.info("For order id {} an outbox message with saga id {} is rolled back",
                    restaurantApprovalResponse.getOrderId(),
                    restaurantApprovalResponse.getSagaId());
            return;
        }
        final var orderApprovalOutboxMessage = approvalOutboxMessageResponse.get();
        final var domainEvent = rollbackOrder(restaurantApprovalResponse);
    ...
    private Order approveOrder(RestaurantApprovalResponse restaurantApprovalResponse) {
        ...
        final var order = orderSagaHelper.findOrder(restaurantApprovalResponse.getOrderId());
    ...
    private OrderApprovalOutboxMessage getUpdatedApprovalOutboxMessage(
        ...
        orderApprovalOutboxMessage.setProcessedAt(
    ...
    private OrderPaymentOutboxMessage getUpdatedApprovalOutboxMessage(
            String sagaId,
            OrderStatus orderStatus,
            SagaStatus sagaStatus) {
        final var orderPaymentOutboxMessageResponse =
                paymentOutboxHelper
        ...
        final var orderPaymentOutboxMessage = orderPaymentOutboxMessageResponse.get();
    ...
    private OrderCancelledEvent rollbackOrder(RestaurantApprovalResponse restaurantApprovalResponse) {
        ...
        final var order = orderSagaHelper.findOrder(restaurantApprovalResponse.getOrderId());
        final var orderCancelledEvent = orderDomainService.cancelOrderPayment(order,
