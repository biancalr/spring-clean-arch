...
public class PaymentOutboxScheduler implements OutboxScheduler {
    ...
    public void processOutboxMessage() {
        ...
        if (outboxMessagesResponse.isPresent() && !outboxMessagesResponse.get().isEmpty()) {
            ...
            log.info("Received {} OrderPaymentOutboxMessages with id {}, sending to message bus",
                    outboxMessages.size(),
                    outboxMessages.stream().map(outboxMessage ->
                            outboxMessage.getId().toString()).collect(Collectors.joining(",")));
            outboxMessages.forEach(outboxMessage ->
                    paymentRequestMessagePublisher.publish(outboxMessage, this::updateOutboxStatus));
            log.info("{} OrderPaymentOutboxMessage is is sent do message bus", outboxMessages.size());
        }
    }
