...
@Slf4j
@Component
public class CustomerKafkaListener implements KafkaConsumer<CustomerRequestAvroModel> {
    ...
    @Override
    @KafkaListener(id = "${kafka-consumer-config.customer-group-id}", topics = "${order-service.customer-request-topic-name}")
    public void receive(@Payload List<CustomerRequestAvroModel> messages,
        ...
        messages.forEach(customerAvroModel ->
                customerMessageListener.customerCreated(orderMessagingDataMapper
                        .customerRequestAvroModelToCustomerModel(customerAvroModel)));
    }
}
